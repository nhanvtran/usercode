...
ClassImp(RooSpinZero_7D) 

RooSpinZero_7D::RooSpinZero_7D(const char *name, const char *title, 
                         RooAbsReal& _m1,
                         RooAbsReal& _m2,
                         RooAbsReal& _h1,
                         RooAbsReal& _h2,
...
RooSpinZero_7D::RooSpinZero_7D(const RooSpinZero_7D& other, const char* name) :  
...
Double_t RooSpinZero_7D::evaluate() const 
{ 
...
Int_t RooSpinZero_7D::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const
{
    if (matchArgs(allVars,analVars,RooArgSet(*h1.absArg(),*h2.absArg(),*hs.absArg(),*Phi.absArg(),*Phi1.absArg()))) return 6 ;
    //if (matchArgs(allVars,analVars,RooArgSet(*m1.absArg(),*m2.absArg(),*h1.absArg(),*h2.absArg(),*Phi.absArg(),*Phi1.absArg()))) return 1 ;
    //if (matchArgs(allVars,analVars,RooArgSet(*m1.absArg(),*m2.absArg(),*h1.absArg(),*h2.absArg(),*hs.absArg(),*Phi.absArg()))) return 2 ;
...
Double_t RooSpinZero_7D::analyticalIntegral(Int_t code, const char* rangeName) const
{
  ...
  */
  ...
  switch(code)
	...
	Double_t term1E = 0;
	Double_t term1F = 0;
	Double_t term1 = betaVal*term1Coeff*term2Coeff*(term1A+term1B+term1C+term1D+term1E+term1F)*4.*TMath::Pi(); 
	// 4pi is the constant for costheta* and Phi1
	...
	return term1*accp;
      ...
      // projection to hs
    case 1:
      return 128.*TMath::Pi()/9.*(f00Val+fmmVal+fppVal)*betaVal*term1Coeff*term2Coeff*2.*TMath::Pi();
      // projection to Phi1
    case 2:
      return 128.*TMath::Pi()/9.*(f00Val+fmmVal+fppVal)*betaVal*term1Coeff*term2Coeff*2.;
    }
    assert(0) ;
    return 0 ;
}
