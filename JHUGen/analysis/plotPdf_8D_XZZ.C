...
void plotPdf_8D_XZZ(double mH = 125, bool draw=true) {
    ...
    // Observables (8D)
    // 
    RooRealVar* zzmass = new RooRealVar("zzmass","m_{ZZ}",mH,100,800);
    ...
    RooRealVar* c1 = new RooRealVar("c1", "c1", 2+2*mV*mV/(mH*mH));
    ...
    RooSpinTwoXZsZs *myPDF = new RooSpinTwoXZsZs("myPDF","myPDF", *zzmass, *z1mass, *z2mass, *hs, *h1,*h2, *Phi, *Phi1, *c1, *phi1, *c2, *phi2, *c3, *phi3, *c4, *phi4, *c5, *phi5, *c6, *phi6, *c7, *phi7, *fz1Val, *fz2Val, *R1Val, *R2Val, *mZ, *gamZ);
    
    // dataset for (JP = 2+)
    TFile* fin = new TFile(Form("TZZ_%.0f_JHU.root", mH));
    TTree* tin = (TTree*) fin->Get("angles");
    RooDataSet data("data","data",tin,RooArgSet(*zzmass, *z1mass, *z2mass, *hs, *h1, *h2, *Phi, *Phi1));
    ...
    if ( draw ) {
      ...
      RooPlot* zzmassframe =  zzmass->frame(55);
      data.plotOn(zzmassframe, LineColor(kBlack));
      // myPDF->plotOn(zzmassframe, LineColor(kRed));
      ...
      czz->SaveAs(Form("epsfiles/angles_TZZ%.0f_JHU_8D.eps", mH));
      czz->SaveAs(Form("pngfiles/angles_TZZ%.0f_JHU_8D.png", mH));
