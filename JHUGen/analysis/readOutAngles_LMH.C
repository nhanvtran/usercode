...
void calculateAngles(TLorentzVector p4H, TLorentzVector p4Z1, TLorentzVector p4M11, TLorentzVector p4M12, TLorentzVector p4Z2, TLorentzVector p4M21, TLorentzVector p4M22, float& costheta1, float& costheta2, float& phi, float& costhetastar, float& phistar1, float& phistar2, float& phistar12, float& phi1, float& phi2);
...
void readOutAngles_LMH(std::string filename, bool applyRes=false){
    ...
    Float_t m_costheta1, m_costheta2, m_phi, m_costhetastar, m_phistar1, m_phistar2, m_phistar12, m_phi1, m_phi2;
    ...
    while (!fin.eof() && fin.good()){
		...
		/*
		cout << "PDGid 0 : " << idup[0] << endl;
		cout << "PDGid 1 : " << idup[1] << endl;
		cout << "PDGid 2 : " << idup[2] << endl;
		cout << "PDGid 3 : " << idup[3] << endl;
		*/
		...
		if(flatype==3){	
		  ...
		  TLorentzVector Graviton = l1_minus+l1_plus+l2_minus+l2_plus;
		
		  const float PDGZmass = 91.2;
		  TLorentzVector pZ1; TLorentzVector pl1_m=l1_minus; TLorentzVector pl1_p=l1_plus;
		  TLorentzVector pZ2; TLorentzVector pl2_m=l2_minus; TLorentzVector pl2_p=l2_plus;
...
TLorentzVector* applyResolution(TLorentzVector *l_gen){
  ...
  if(randomForSmearing.Uniform()<.9){
    ...
    float l_Theta = l_gen->Theta()+(randomForSmearing.Gaus(0,0.001));
    float l_Phi = l_gen->Phi()+(randomForSmearing.Gaus(0,0.001));
  }else{
    float l_Perp =
      l_gen->Perp()+randomForSmearing.Gaus(-l_gen->Perp()*.04,l_gen->Perp()*.08);
    //float l_Perp = l_gen->Perp();
    float l_Theta = l_gen->Theta()+(randomForSmearing.Gaus(0,0.001));
    float l_Phi = l_gen->Phi()+(randomForSmearing.Gaus(0,0.001));
  }
