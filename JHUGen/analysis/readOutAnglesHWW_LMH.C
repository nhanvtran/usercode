...
void readOutAnglesHWW_LMH(std::string filename){
  ...
  int maxEvents = 100000; // max 100K events
  ...
  Double_t m_costheta1, m_costheta2, m_phi, m_costhetastar, m_phistar1, m_phistar2, m_phistar12, m_phi1, m_phi2, m_psi;
  ...
  while (!fin.eof() && fin.good()){
	...
	std::cout << Form("nubar (px, py, pz, E): (%.3f, %.3f, %.3f, %.3f) \n", nubar.Px(), nubar.Py(), nubar.Pz(), nubar.E());
      */
      ...
      double angle_costheta1, angle_costheta2, angle_phi, angle_costhetastar, angle_phistar1, angle_phistar2, angle_phistar12, angle_phi1, angle_phi2, angle_psi;
