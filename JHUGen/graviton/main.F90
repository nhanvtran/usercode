...
implicit none
character :: arg*(31)
integer :: NumArgs,NArg,OffShell_HZZ

   Collider=1
   VegasIt1=-1
   PDFSet=1      ! 1: CTEQ6L1   2: MRSW with best fit, 2xx: MRSW with eigenvector set xx=01..40
   VegasNc1=-1
   PChannel=2
   ...
   Unweighted =.true.
   OffShell_HZZ=000! 000: H,Z1,Z2 on-shell; 010: H,Z2 on-shell, Z1 on-shell; and so on
...
!         read(arg(10:11),*) VegasIt1
    ...
    elseif( arg(1:7) .eq."OffHZZ=" ) then
        read(arg(8:10),*) OffShell_HZZ
    endif
   enddo
    ...
    if(OffShell_HZZ.ge.100) then
        OffShell_HZZ=OffShell_HZZ-100
        OffShellHiggs=.true.
    else
        OffShellHiggs=.false.
    endif
    if(OffShell_HZZ.ge.10) then
        OffShell_HZZ=OffShell_HZZ-10
        OffShellZ1=.true.
    else
        OffShellZ1=.false.
    endif
    if(OffShell_HZZ.ge.1) then
        OffShell_HZZ=OffShell_HZZ-1
        OffShellZ2=.true.
    else
        OffShellZ2=.false.
    endif
    ...
    if( (OffShellZ1.or.OffShellZ2.or.OffShellHiggs) .and. Process.ne.0) then
        print *, "off shell Z's only allowed for spin 0 resonance"
        stop
    endif
...
integer :: AllocStatus,NHisto
  ...
  it_sav = 1
          ...
          Histo(6)%NBins  = 200
          Histo(6)%BinSize= 0.5d0/100d0
          Histo(6)%LowVal = 10d0/100d0
          Histo(6)%SetScale= 100d0

          Histo(7)%Info   = "MZ2"   ! Z1 invariant mass
          Histo(7)%NBins  = 200
          Histo(7)%BinSize= 0.5d0/100d0
          Histo(7)%LowVal = 10d0/100d0
          Histo(7)%SetScale= 100d0


          Histo(8)%Info   = "MG"   ! Grav invariant mass
          Histo(8)%NBins  = 200
          Histo(8)%BinSize= 2d0/100d0
          Histo(8)%LowVal = 10d0/100d0
