...
void DrawLimit(char*channel, char* model="unbin", double el_lumi=19.2, double mu_lumi=19.3, bool showobs=1, char* theory_model="BulkG")//"SM"
{
	...
	TMultiGraph *likelihd_limit = new TMultiGraph("exclusionlimit_p",";m_{G} [GeV]; 95% CL limit on #sigma#times B(fb)" );  
	...
	for(Int_t i=0;i<20;i++){
		...
		xs_theory_smooth[i]=XS4Graviton("BulkG",mass_smooth[i]/1000.,0.2)*xs_theory_scale;
		cout<<"mass="<<mass_smooth[i]<<" xs="<<xs_theory_smooth[i]<<endl;
	}
	//TGraph *gr_xs_theory = new TGraph(nmass,Mass,XS_theory); 
	TGraph *gr_xs_theory = new TGraph(20, mass_smooth, xs_theory_smooth); 
	gr_xs_theory->SetLineColor(kRed);
